...
@pytest.mark.asyncio
async def test_update_user_success(override_get_db, test_db_session):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.patch(
            f"/api/users/{user.id}", json={"username": "updateduser"}
        )
...
@pytest.mark.asyncio
async def test_delete_user_success(override_get_db, test_db_session):
    ...
    deleted_user = await test_db_session.get(User, user_id)
